...
def render(
    ...
    # Serve template via local HTTP so ES module imports from esm.sh work
    ...
    httpd = http.server.HTTPServer(("127.0.0.1", port), lambda *a, **k: handler(*a, directory=template_dir, **k))
    ...
    with sync_playwright() as p:
        try:
            browser = p.chromium.launch(headless=True, args=["--no-sandbox", "--disable-setuid-sandbox", "--disable-web-security", "--allow-running-insecure-content"])
